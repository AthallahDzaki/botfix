...
module.exports = async (msg, grace) => {
  ...
  if (!msg.member.voice.channelID) {
    msg.reply('you need to be in a voice channel :p').catch(() => {});
    return;
  }
  if (!singleArgument) {
  ...
  if (!msg.guild.voiceConnection) {
    await memberVC.join().then(() => song.play())
      .catch(() => msg.reply('couldn\'t join the voice channel!').catch(() => {}));
  } else {
